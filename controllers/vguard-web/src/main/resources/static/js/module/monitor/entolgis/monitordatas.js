 ...
 * 根据查询条件获得企业信息
 * */
monitordatas.getEnterprises = function (entname, districtcode, businessinfoid) {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: 'json',
        data: {"entName": entname, "districtcode": districtcode, "businessinfoid": businessinfoid},
        global: false,
        async: false,
        success: function (map) {
            if (map.length == 0 || map == null) {
                window.wxc.xcConfirm("该企业没有安装视频或探头！", window.wxc.xcConfirm.typeEnum.info);
            } else {
                data = map;
            }
        },
 ...
 * */
monitordatas.loadVideoInfo = function (videoid) {
    var data;
    $.ajax({
        type: 'post',
 ...
 * 获得报警数目
 * */
monitordatas.getAlarmCount = function () {
    var count;
    $.ajax({
        type: 'post',
