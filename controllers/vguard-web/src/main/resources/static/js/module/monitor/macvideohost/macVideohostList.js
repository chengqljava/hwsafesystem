$(document).ready(function() {
	var colname = ['主机id','主机编号','主机品牌','主机型号','主机名称','用户名','密码','IP','端口号', '所属组织', '视频类型', '所属企业', '所属海康平台', '状态','备注']; 
	var colmodel = [
					...
					{name: 'GOVVIDEOTYPE', index: 'GOVVIDEOTYPE', align: 'left',
						formatter: function(cellvalue, options, obj) { 
							if ("0" == obj.GOVVIDEOTYPE) {
								return "园区";
							} else if ("1" == obj.GOVVIDEOTYPE) {
								return "高空聊望";
							} else {
								return "-";
							}
					...
					{name: 'HIKPLATNUM', index: 'HIKPLATNUM', align: 'left'},
...
$("#addBtn").on("click", function () {
	parent.openWin(BASE_URL+"/monitor/macvideohost/add",'添加','70%','70%');
});
...
$("#editBtn").on("click", function () {
	...
	parent.openWin(BASE_URL+'/monitor/macvideohost/edit/'+VIDEOHOSTID,'编辑','70%','70%');
});

/*详细查询*/
function display(VIDEOHOSTID){
	parent.openWin(BASE_URL+"/monitor/macvideohost/display/"+VIDEOHOSTID,'详细','70%','70%');
}
/**
 * 选择一条视频主机
 */
$("#entbtn").on("click",function(){
	...
	paramsJson+="'HIKPLATNUM':'"+rowdata.HIKPLATNUM+"',";
	paramsJson+="'IPADDR':'"+rowdata.IPADDR+"',";
	paramsJson+="'PORT':'"+rowdata.PORT+"'";
	paramsJson+="}";
	paramsJson+="]";
