 ...
 * 选择执法人员
 */
$("#selLawUserBtn").off("click").on("click", function(){
	// 返回当前grid中复选框所选择的数据的id
    var curSelRowArr = $("#grid-table").jqGrid("getGridParam", "selarrrow");
    if (1 != curSelRowArr.length) {
        // 弹出提示信息
        parent.toast("请选择一个需要选择的执法人员！");
        return;
    }
    
    var curRowDataObj = $("#grid-table").jqGrid("getRowData", curSelRowArr[0]);
    //给父窗口中企业赋值方法
    var index = parent.getParentIndex();
	parent.frames["layui-layer-iframe"+index].setLawUserInfo({
		"flag": $("#flag").val(),
		"lawUserId": curRowDataObj.USERID,
		"lawUserName": curRowDataObj.USERNAME
	});
    parent.closeWin();
});