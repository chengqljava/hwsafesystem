$(document).ready(function() {
	...
	var colname = ['主键id','队伍编号', '队伍名称', '填报单位', '总人数', '主要负责人', '应急值班电话', '填报状态','地理定位',"操作"]; 
	...
	var colmodel = [
            		...
            		{name:'OPERATION',index:"OPERATION",width:'80px',fixed:true,align:"left",formatter:function(cellvalue, options, obj){
            			return '<a href="javascript:void(0);" title="编辑" class="operation editInfoBtn" onclick="editInfo(\''+obj.TEAMID+'\')">编辑</a><br>'
            			     + '<a href="javascript:void(0);" title="删除" class="operation delInfoBtn" onclick="delInfo(\''+obj.TEAMID+'\')">删除</a>'
            		}}
			];
    ...
    $("#grid-table").jqGrid({
    	height: tableHeight,
    	url : BASE_URL + "/ems/emsresteam/list",
		datatype: "json",
		cache : false,
		mtype : 'post',
		colNames:colname,
...
function reloadGrid(){
	...
	if ($("#teamrank").val() == '国家级') {
		searchParamJson.teamrank = '1';
	}else if($("#teamrank").val() == '省级'){
		searchParamJson.teamrank= '2';
	}else if($("#teamrank").val() == '地市级'){
		searchParamJson.teamrank = '3';
	}else if($("#teamrank").val() == '县级'){
		searchParamJson[teamrank] = '4';
	}else if($("#teamrank").val() == '企业级'){
		searchParamJson.teamrank = '5';
	}
	if ($("#teamstats").val() == '专业应急救援队伍') {
		searchParamJson.teamstats = '1';
	}else if($("#teamstats").val() == '兼职应急救援队伍'){
		searchParamJson.teamstats = '2';
	}else if($("#teamstats").val() == '志愿者应急救援队伍'){
		searchParamJson.teamstats = '3';
	}
	var searchParamJson = JSON.stringify(searchParamJson);
	var searchParam = {"searchParamJson" : searchParamJson};
