$(document).ready(function() {
    ...
    $("#grid-table").jqGrid({
    	height: tableHeight,
    	url : BASE_URL + "/hiddendanger/hdigovregistra/gov/listbyent",
		datatype: "json",
		cache : false,
		mtype : 'post',
		colNames:colname,
...
function display(registraid){
	parent.parent.openWin(BASE_URL+'/hiddendanger/hdigovregistra/display/'+registraid,'隐患详细','','85%');
}

/*企业详细信息*/
function entDisplay(businessinfoid,entname) {
	// parent.parent.openWin(BASE_URL+'/enterprise/entsafeinfomenu/safemenuDisplay/'+businessinfoid,entname,'80%','90%');
...
$("#reviewBtn").on("click", function () {
	...
	if(statusstr != 1){
		// 弹出提示信息
		parent.parent.toast("只能对已整改的隐患进行复查！");
		return;
	} 
	parent.parent.openWin(BASE_URL+'/hiddendanger/hdigovreview/review/'+registraid+'/'+businessinfoid,'复查','60%','85%');
...
$("#clcaseBtn").on("click", function () {
	...
	if(statusstr != 3){
		// 弹出提示信息
		parent.parent.toast("只能对已复查的隐患进行销案！");
		return;
	} 
	if(reviewresults == 0){
		// 弹出提示信息
		parent.parent.toast("复查不合格不能进行销案！");
		return;
	}
	parent.parent.openWin(BASE_URL+'/hiddendanger/hdigovcancelcase/closecase/'+registraid+'/'+businessinfoid,'销案','40%','40%');
