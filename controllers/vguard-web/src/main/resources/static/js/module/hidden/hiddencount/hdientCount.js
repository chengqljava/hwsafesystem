...
function HdiEntCount(){
	$.ajax({
		type : 'post',
		url : BASE_URL+/*'/hiddendanger/hdientcount/report'*/'/hidden/hidcount/report',
		cache : false,
		...
		success : function(map) {  
			if(map.success==true){
				if (map.data.entqty > 0) {
					...
					$("#entqty").html(map.data.entqty);
					$("#entqty2").html(map.data.entqty);
					$("#selfcheckedqty").html(0);
					$("#noselfcheckqty").html(0);
					$("#selfcheckingqty").html(0);
					$("#selfcheckrate").html(0);
					$("#selfhdientqty").html(0);
					$("#selfhdiqty").html(0);
					$("#selfhdirate").html(0);
					$("#selfreformedqty").html(0);
					$("#noselfreformqty").html(0);
					$("#selfreformingqty").html(0);
					$("#selfreformrate").html(0);
					$("#patroledqty").html(0);
					$("#nopatrolqty").html(0);
					$("#patrolrate").html(0);
					$("#patrolhdientqty").html(0);
					$("#patrolhdiqty").html(0);
					$("#patrolhdirate").html(0);
					$("#patrolreformedqty").html(0);
					$("#nopatrolreformqty").html(0);
					$("#patrolreformingqty").html(0);
					$("#patrolreformrate").html(0);
				}
			}
		}
	});
...
function getEntList(){
	var etime = $("#etime").val();
	var stime = $("#stime").val();
	parent.openWin(BASE_URL+"/hidden/hidcount/dispalyentlist?etime="+etime+"&stime="+stime+"&flag=all",'监管企业','80%','60%');
}
...
function getSelfCheckEntList(checkresult){
	...
	if (checkresult == 1 ) {
		titlename = "已自查列表"
		parent.openWin(BASE_URL+ "/views/module/hidden/hidcount/govCheckRecordList.html?checkresult="+checkresult+"&stime="+stime+"&etime="+etime+"&isgov=0",titlename,'80%','60%');
	}else if(checkresult == 2){
	 titlename = "自查中列表"
		 parent.openWin(BASE_URL+ "/views/module/hidden/hidcount/govCheckRecordList.html?checkresult="+checkresult+"&stime="+stime+"&etime="+etime+"&isgov=0",titlename,'80%','60%');
	}else {
		titlename = "未自查列表"
		parent.openWin(BASE_URL+"/hidden/hidcount/dispalyentlist?etime="+etime+"&stime="+stime+"&flag=not&isgov=0",titlename,'80%','60%');
	}
} 
 ...
 * 隐患企业列表
 */
function getSelfCheckHdEntList(isgov){
	var titlename;
	if (isgov == 0) {
		titlename = '自查隐患企业列表'
	}else {
	...
	var stime = $("#stime").val();
	var etime = $("#etime").val();
	parent.openWin(BASE_URL+"/hidden/hidcount/dispalyentlist?etime="+etime+"&stime="+stime+"&flag=hid&isgov="+isgov,titlename,'80%','60%');
}
...
function getEntDistrictCountList(){
	...
	parent.openWin(BASE_URL+"/hidden/hidcount/displayentdistrictcountlist?begindate="+stime+"&enddate="+etime+"&districtid="+districtid,'安全隐患排查治理情况汇总表（块网格）','80%','60%');
}

/**
 * 自查隐患列表
 */
function getCheckHdList(reformstate){
	var stime = $("#stime").val();
	var etime = $("#etime").val();
	var isgov;
	if (!reformstate.indexOf('ent')) {
		isgov = '0';
	}else if(reformstate ==''){
		isgov = '';
	}else{
	...
	parent.openWin(BASE_URL+"/views/module/hidden/hidcount/govHiddendangerList.html?reformstate="+reformstate+"&etime="+etime+"&stime="+stime+"&isgov="+isgov,'隐患列表','80%','60%');
}
...
function getPatrolEntList(patrolstate){
	...
	var etime = $("#etime").val();
	if (patrolstate == "0") {
		parent.openWin(BASE_URL+"/hidden/hidcount/dispalyentlist?etime="+etime+"&stime="+stime+"&flag=not&isgov=1",'未巡查企业','80%','60%');
	}else{
		parent.openWin(BASE_URL+ "/views/module/hidden/hidcount/govCheckRecordList.html?stime="+stime+"&etime="+etime+"&isgov=1",'已巡查企业','80%','60%');
	}
}
...
function EntSelfCheckChart(){
	...
	$.ajax({
		type : 'post',
		url : BASE_URL+'/hidden/hidcount/selfcheckchart',
		cache : false,
		data:$("#hdientcountform").serializeArray(),
		dataType : 'json',
		global : false,
...
function EntPatrolChart(){
	...
	$.ajax({
		type : 'post',
		url : BASE_URL+'/hidden/hidcount/patrolchart',
		cache : false,
		data:$("#hdientcountform").serializeArray(),
		dataType : 'json',
		global : false,
...
function EntSelfCheckHdChart(){
	...
	$.ajax({
		type : 'post',
		url : BASE_URL+'/hidden/hidcount/selfcheckhdchart',
		cache : false,
		data:$("#hdientcountform").serializeArray(),
		dataType : 'json',
		global : false,
...
function EntPatrolHdChart(){
	...
	$.ajax({
		type : 'post',
		url : BASE_URL+'/hidden/hidcount/patrolhdchart',
		cache : false,
		data:$("#hdientcountform").serializeArray(),
		dataType : 'json',
		global : false,
