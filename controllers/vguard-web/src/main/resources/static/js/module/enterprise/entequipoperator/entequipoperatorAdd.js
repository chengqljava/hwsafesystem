$(document).ready(function() {
	...
	SelectOption.loadSex("sex"); // 性别
	 //初始化特种作业岗位下拉树
	SelectTree.loadSpecialEquipPositionSelect("positionname");

	$("#equipoperatorform").validate({
		rules: {
			operatorname: {
				required: true,
				rangelength:[1,25]
...
/**
	$('.form_datetime').datetimepicker({
		...
		$('#equipoperatorform').formValidation({
...
function save(){
	$.ajax({
		type : 'post',
		url : BASE_URL+'/enterprise/entequipoperator/save',
		cache : false,
		dataType : 'json',
		data : $("#equipoperatorform").serializeArray(),
		global : false,
		success : function(json) {
			if(json.success==true){
				...
				parent.frames["layui-layer-iframe"+index].$('#ent_equipoperator').val("true");
