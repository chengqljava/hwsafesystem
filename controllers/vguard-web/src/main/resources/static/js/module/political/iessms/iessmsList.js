$(document).ready(function() {
	var colname = ['主键ID','标题','发送人','发送时间','发送人号','接收人','接收人号']; 
    ...
    $("#grid-table").jqGrid({
    	height: tableHeight,
    	url : BASE_URL + "/political/iessms/list",
		datatype: "json",
		cache : false,
		mtype : 'post',
		colNames:colname,
...
$("#addBtn").on("click", function () {
	parent.openWin(BASE_URL+"/political/iessms/add/" + $('#datatype').val(),'添加','65%','65%');
});
...
$("#editBtn").on("click", function () {
	...
	parent.openWin(BASE_URL+'/political/iessms/edit/'+smsid+'/'+datatype,'编辑','65%','65%');
});


/*详细查询*/
function display(SMSID){
	parent.openWin(BASE_URL+"/political/iessms/display/"+SMSID,'详细','65%','65%');
}
...
function del(param){
	...
	parent.confirm('确认删除吗?',function(){
		$.ajax({ 
	  		url: BASE_URL+"/political/iessms/delete",
	  		type:'post',
	  		dataType:'json',
	  		data:param,
