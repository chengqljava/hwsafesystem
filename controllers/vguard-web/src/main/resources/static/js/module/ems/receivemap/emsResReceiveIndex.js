...
//	metisMenuFmt();
	...
	$("#taskReceiveBtn").show();			
	//绑定GIS首页右上角任务接收按钮事件
	$("#taskReceiveBtn").off("click").on("click", function(e) {
		//任务接收按钮弹窗
		openWin(BASE_URL + "views/module/ems/receivemap/taskReceiveList.html", "任务列表", "80%", "80%", true, true);
		return false;
	});
...
//				num = renderLTSubGrid("yjjy", "");
		...
		switch (text) {
			...
			case "事故模拟":
				...
				openEmsResWin(BASE_URL + "views/module/ems/map/evnanalog/evnAnalogIndex.html?" +
							  ...
							  "&eventlatitude=" + $("#eventlatitude").val(), "事故模拟", "45%", "400px", "sgmn", function() {
					//清除事故模拟相关的覆盖物
	...
	$("#caseSwitchBtn").off("click").on("click", function() {
		//事件信息弹窗
		openWin(BASE_URL + "views/module/ems/map/eventInfo/eventInfoList.html", "当前上报事故信息", "60%", "60%", true, true);
		return false;
	}); 
...
function initDriveRoute(data,type) {
	/*var color = new Array('blue','#ff7f50','#87cefa','#da70d6','#32cd32','#6495ed',
		...
		var driving = new BMap.DrivingRoute(window.map,
				{
					...
					onMarkersSet: function(pois) {
						...
						_.map(pois, function(poi) {
							window.allDriveRouteArr.push(poi.marker);
							if(type=="jylx"){
								poi.marker.setTitle(curdata.ROUTENAME+"的救援路线");
							}else if(type=="cllx"){
								poi.marker.setTitle(curdata.ROUTENAME+"的撤离路线");
							}														
						});						
					},
					renderOptions:{map: window.map,panel: "r-result", autoViewport: true}
...
//查询任务接收提醒
function loadTaskWaiting(){
	//加载请求获取待办事项
	$.ajax({
		type :'post',
		url : BASE_URL+'/ems/emssucisstask/taskmsglist',
		cache : false,
		dataType : 'json',
		global : false,
		async : false,
		success : function(result) {
			//更新右上角任务数量值
			$("#taskcount").text(result.taskcount);
			if(result.tasklist.length>0){
				var content = "";
				var nodename;
				_.map(result.tasklist,function(task){
					switch(task.node){
						case 1:
							nodename = "应急响应";
							break;
					...
					if(nodename.length>5){
						content+='<a href="javascript:void(0);" title="任务下发：'+getSmpFormatDateByLong(task.receivetime,true)+'&nbsp;'+nodename+'" onclick="initTodo(\''+task.taskid+'\',\''+task.node+'\',\''+getSmpFormatDateByLong(task.receivetime,true)+'\',\''+task.taskstatus+'\',\''+task.taskreceiver+'\',\''+task.schemeid+'\',\''+nodename+'\',\''+task.taskcontent+'\')"><div class="tip03">任务下发：'+getSmpFormatDateByLong(task.receivetime,true)+'&nbsp;'+nodename.substring(0,4)+'...</div></a>'; //捕获的元素
					}else{
						content+='<a href="javascript:void(0);" title="任务下发：'+getSmpFormatDateByLong(task.receivetime,true)+'&nbsp;'+nodename+'" onclick="initTodo(\''+task.taskid+'\',\''+task.node+'\',\''+getSmpFormatDateByLong(task.receivetime,true)+'\',\''+task.taskstatus+'\',\''+task.taskreceiver+'\',\''+task.schemeid+'\',\''+nodename+'\',\''+task.taskcontent+'\')"><div class="tip03">任务下发：'+getSmpFormatDateByLong(task.receivetime,true)+'&nbsp;'+nodename+'</div></a>'; //捕获的元素
					}
				});
				//捕获页
		        return layer.open({
...
function initTodo(taskid,node,receivetime,taskstate,receiver,schemeid,nodename,taskcontent){
	parent.taskContentInfo = taskcontent;
	var url = BASE_URL+"views/module/ems/receivemap/respTask.html?taskid="+taskid+"&node="+node+"&receivetime="+receivetime+"&taskstate="+taskstate+"&receiver="+receiver+"&schemeid="+schemeid+"&ticket=";
	parent.openWin(url,nodename,"55%","60%");
	/*var app = window.Mirs.Application.3();
	console.log(app);
	var cwin = app.getWorkControl().getMdiControl().createChildWindow();
	var iframeEle = document.createElement("iframe");
	var hostName = window.location.hostname;*/
	/*iframeEle.src = url + "45679686-90DF-42EB-8BDA-BE57F8963A64";
	iframeEle.style.width = "100%";
	iframeEle.style.height = "100%";
	iframeEle.id = "TodoMessage";
	iframeEle.name = "TodoMessage";
	cwin.setIframeElement(iframeEle);
	cwin.getContainer().appendChild(iframeEle);
	cwin.setCaption("消息提醒");
