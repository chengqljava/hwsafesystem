...
//	SelectOption.loadEquipmenttype("equipmenttype");
    ...
    //生成任务列表分页表格
    var colname = ["维修id","设备类型","设备编号","设备名称","维修人员","联系电话","维修时间", "故障原因","维修方式"/*,"登记情况"*/],
        colmodel = [
            {
                name: "MAINTAINID",
                index: "MAINTAINID",
  ...
  //分页表格响应式处理
    ...
    $("#addBtn").off("click").on("click", function (){
    	parent.openWin(BASE_URL
				+ "views/module/monitor/macmaintan/macmaintanAdd.html?maintainid=-1",
				'新增监控监测设备维修', '50%', '58%');
    });
    ...
    $("#editBtn").off("click").on("click", function (){
        ...
        if(isreport == '未上报'){         	
        	var maintainid = $("#grid-table").jqGrid("getRowData", curSelRowArr[0]).MAINTAINID;
        	//TODO 弹出编辑框
        	parent.openWin(BASE_URL
        			+ "views/module/monitor/macmaintan/macmaintanAdd.html?maintainid="+maintainid,
        			'编辑监控监测设备维修', '50%', '58%');
        } else{
        	// 弹出提示信息
...
//    
    ...
    function delMaintains(param) {
        ...
        parent.confirm("确认删除吗?", function () {
            $.ajax({
                url: BASE_URL + "monitor/macmaintan/delete",
                type: "post",
                dataType: "json",
                data: param,
    ...
    function upMaintains(param) {
        ...
        parent.confirm("上报之后不能再修改，确认上报吗？", function () {
            $.ajax({
                url: BASE_URL + "monitor/macmaintan/audit",
                type: "post",
                dataType: "json",
                data: param,
                success: function (json) {
