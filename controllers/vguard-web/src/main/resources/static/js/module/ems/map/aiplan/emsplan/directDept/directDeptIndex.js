...
//		alert("schemaId" + schemaId);
	    ...
	    $(window).resize(function(){
	        $("#deptGrid-table").jqGrid( 'setGridHeight', 140 );
	        $("#deptGrid-table").jqGrid( 'setGridWidth', $(window).width() - 30 );
	    });
	    ...
	    $("#deptGrid-table").jqGrid({
	        ...
	        loadComplete: function() {
	        	if($(window).width() < 700) {
					...
					$("#deptGrid-table").closest(".ui-jqgrid-bdiv").css({ "overflow-x" : "scroll"});
...
function displayOrg(orgid){
    parent.openWin(BASE_URL+"/ems/emsresorg/display/"+orgid,'详细','58.5%','85%');
}