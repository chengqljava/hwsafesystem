...
$(document).ready(function() {
	...
	0 < $("#plantype").length && SelectOption.loadEmsPlanType("plantype");
//	SelectOption.loadEmsPlanType("plantype");
	...
	//判断是否系统首页的预览政府各类型预案列表页面
	var curPlanType = null,
		initYaType = $("#yaType").val();
	if ("0" == initYaType) {
		curPlanType = $("#plantype").val();
	} else {
		curPlanType = initYaType;
	} 
	...
	if ("0" != businessinfoId) {
		gridParam.businessinfoId = businessinfoId;
	}
	
    $("#grid-table").jqGrid({
    	height: tableHeight,
    	url : BASE_URL + "/ems/emsplaplaninfo/mainentlist",
		datatype: "json",
		cache : false,
		mtype : 'post',
		colNames:colname,
...
$("#resetbtn").bind("click",function(){
	$("#planname").val("");
	0 < $("#plantype").length && $("#plantype").val("");
	$("#planstate").val("");
	$("#districtid").val("");
});
