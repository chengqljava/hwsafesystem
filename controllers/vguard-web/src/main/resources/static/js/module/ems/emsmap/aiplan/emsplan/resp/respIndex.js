...
//	alert("schemaId" + schemaId);
	if ("0" != schemaId) {
		...
		$(window).resize(function(){
			$("#respGrid-table").jqGrid( 'setGridHeight', $(window).height() - $('.pcheck').height() - 190 -33 );
	    ...
	    $("#respGrid-table").jqGrid({
			...
			loadComplete: function() {
				if($(window).width() < 700) {
					...
					$("#respGrid-table").closest(".ui-jqgrid-bdiv").css({ "overflow-x" : "hidden"});
//					$(".ui-jqgrid-hdiv").css({ "overflow-x" : "scroll"});
					$(".table-line .tableTitle").find("button").css({"margin-right":"0px","margin-top":"7px"});
				} else {
//					$("#respGrid-table").jqGrid( 'setGridWidth', $(window).width()*0.99 );
				}
				
				$("#respGrid-table").jqGrid( 'setGridHeight', $(window).height() - $('.pcheck').height() - 190 );
    ...
    $("#grid-table").jqGrid({
        ...
        loadComplete: function() {
			if($(window).width() < 400) {
				...
				$("#grid-table").closest(".ui-jqgrid-bdiv").css({ "overflow-x" : "hidden","height":"350"});
