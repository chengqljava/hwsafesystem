$(document).ready(function() {
	...
	var colname = ['主键id','和gis关联ID','应急仓库id','物资设备编号','物资设备名称','行政区域','物资设备类别大类','物资设备类别小类','规格型号','数量','所属仓库','地理定位']; 
	...
	/*if($("#entUser").val()){
        //企业
       colname.push("状态");
       colmodel.push({name:'FILLSTATE',index:'FILLSTATE',width:'5%',
                        formatter:function(cellvalue, options, obj) { 
    ...
    $("#grid-table").jqGrid({
    	height: tableHeight,
    	url : BASE_URL + "/ems/emsresmaterial/materialList",
		datatype: "json",
		cache : false,
		mtype : 'post',
		colNames:colname,
