$(document).ready(function() {
	...
	if(undefined==$('#type').val()){
		...
		var colname = ['主键id','执法登记id','案件编号','案件名称','被调查企业','所属辖区','执法部门','执法人员','立案日期','结案时间','案件状态','最近更新时间'];
		var colmodel = [
			...
			{name:'CASENAME',index:'CASENAME',width:'15%',align:'center',sortable : false,
				formatter:function(cellvalue, options, obj) { 
					   return '<a href="javascript:void(0);" onclick="displayCase(\''+obj.CHECKINFOID+'\',\''+obj.CASEID+'\',\''+obj.CASENAME+'\',\''+obj.PROCESS+'\')">'+obj.CASENAME+'</a>';
				}
			},
			...
			{name:'CASESTATE',index:'CASESTATE',width:'10%',align:'center',sortable : false,
				editoptions : {
					value : "1:已立案;2:审理中;3:执行中;4:已结案;5:已中止"
				},
				formatter : 'select'
			},*/
			{name:'CASESTATE',index:'CASESTATE',width:'10%',align:'center',
				formatter:function(cellvalue, options, obj) { 
					if (obj.CASESTATE=='4' && obj.PROCESS=='1') {
						return "已结案(简易程序)";
					} else if (obj.CASESTATE=='4') {
						return "已结案";
					} else {
						return "";
					}
...
/*详细查询*/
function displayCase(checkinfoid,caseid,casename,process){
	if(process=='1'){
		parent.openWin(BASE_URL+"/law/lawdocinfo/menu?id="+checkinfoid+"&doctype=1,2&menupagetype=menuDisplay&caseid=" + caseid +"&writtype=null",casename,'80%','98%');	
	}else{
...
$("#addBtn").bind("click", function() {
	parent.openWin(BASE_URL + '/law/lawcase/add', '添加','40%','30%');
});
