	...
	$.ajax({
		...
		success : function(map) {
			if (map.success == true) {
				...
				$("#transtoolCount").html(map.data.transtoolCount);
				$("#comfirmCount").html(map.data.comfirmCount);
...
function getEmsResource(checkresult) {
	...
	if (checkresult == "1") {
		...
		parent.openWin(BASE_URL
				+ "/views/module/ems/emsrescomfirm/emsrescomfirmList.html?",
				'通讯保障列表', '80%', '70%');
	}else if(checkresult == "9"){
		parent.openWin(BASE_URL
				+ "/views/module/ems/emsrestranstool/emsrestranstoolList.html?",
				'运输保障列表', '80%', '70%');
	}
}
...
function EntSelfCheckChart() {
	var stautsType = [ '应急机构', '应急队伍', '应急专家', '应急仓库', '应急物资', '医疗机构', '避难场所', '运输保障', '通讯保障' ];
	$.ajax({
		type : 'post',
