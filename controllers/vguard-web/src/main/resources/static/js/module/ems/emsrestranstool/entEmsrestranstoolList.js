$(document).ready(function () {
    ...
    //生成任务列表分页表格
			var colname = ["主键id","运输工具编号","运输工具名称","日常用途","存放地点","载重量", "载人数","主管单位","负责人","填报状态","操作"],
	        colmodel = [
	            {
	                name: "TRANSTOOLID",
	                index: "TRANSTOOLID",
	            ...
	            },
	            {
         		...
         		{name:'OPERATION',index:"OPERATION",width:'80px',fixed:true,align:"left",formatter:function(cellvalue, options, obj){
        			return '<a href="javascript:void(0);" title="编辑" class="operation editInfoBtn" onclick="editInfo(\''+obj.TRANSTOOLID+'\')">编辑</a><br>'
        			     + '<a href="javascript:void(0);" title="删除" class="operation delInfoBtn" onclick="delInfo(\''+obj.TRANSTOOLID+'\')">删除</a>'
        		}}
	        ];
...
function delBadrecords(param) {
    //弹出提示框
    parent.confirm("确认删除该运输保障?", function () {
        $.ajax({
