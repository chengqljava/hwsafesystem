...
function areaBecome(imgRealHeight,imgRealWidth,n){
	...
	$("svg polygon").each(function (e) {
		...
		allPoint.forEach(function(item,index){
			if(index%2 == 0){
        		allpoints.push(item*imgRealWidth/100*n);            		
        	}
        	else{
        		allpoints.push(item*imgRealHeight/100*n);
        	}
		})
	...
	$(".point").each(function (e) {
		var dataTop = $(this).attr("data-top");
		var dataLeft = $(this).attr("data-left");
		var top = imgRealHeight*dataTop * n/100 -80;
		var left = imgRealWidth*dataLeft * n/100 -30;
		$(this).css({top: top, left: left});
	});
}
