$(document).ready(function() {
	var colname = ['复议诉讼id','单位名称','案件名称','复议主持人','申请人','申请时间','审批状态']; 
	var colmodel = [
...
$("#addBtn").on("click", function () {
	parent.openWin(BASE_URL+"/law/lawrec/add",'添加','70%','80%');
});
...
$("#editBtn").on("click", function () {
	...
	parent.openWin(BASE_URL+'/law/lawrec/edit/'+lawsuitid,'编辑','70%','80%');
});

/*详细查询*/
function display(lawsuitid){
	parent.openWin(BASE_URL+"/law/lawrec/display/"+lawsuitid,'详细','70%','80%');
}
...
function del(param){
		...
		parent.confirm("确认删除吗?", function() { 
			 $.ajax({ 
				 	url: BASE_URL+"/law/lawrec/delete",
			  		type:'post',
			  		dataType:'json',
			  		data:param,
