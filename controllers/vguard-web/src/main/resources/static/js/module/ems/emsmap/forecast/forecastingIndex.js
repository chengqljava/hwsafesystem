...
var dlfsAndjjqBtn = true;//fasle : 新增 显示 新增 警戒区  道路    true：不显示

var overlays = new Array();
$(document).ready(function () {
    $('body').on('click', '#foreAdd', function () {
        $('#foreAddBtn').hide();
        $('.table-line').hide();
        $('#foreTitle').show();
        $('.div_left').show();
...
//    clickFirst();//自动触发点击防护目标
    ...
    $("#foreAdd").click(function () {
        ...
        if (parent.window.allDriveRouteArr) {
            if (0 < parent.window.allDriveRouteArr.length) {
                _.map(parent.window.allDriveRouteArr, function (tmpPolyObj, index) {
                	parent.window.map.removeOverlay(tmpPolyObj);
                });
            }
           parent.window.allDriveRouteArr = [];
        }
        clickFirst();//自动触发点击防护目标
...
//                            parent.simpleMarker(raodname, e.point.lng, e.point.lat);
                            $("#contentIframe").attr("src", BASE_URL + "views/module/ems/emsmap/forecast/roadInfo.html");
                            $("#roadname").val("");
...
//        autoHeight();
    ...
    //设置警戒区域
    $("#jjqBtn").off("click").on("click",function () {
        ...
        var overlaycomplete = function (e) {
            ...
            if (isNaN(area)) {
                ...
                parent.window.map.removeOverlay(e.overlay);
                return false;
            }
            if (Number(area / 10000000) > 1) {
            	parent.toast("警戒区域面积过大,请重新绘制!");
            	parent.window.map.removeOverlay(e.overlay);
            	return false;
            }
        ...
        $("#contentIframe").attr("src", BASE_URL + "views/module/ems/emsmap/forecast/warnArea.html");
...
function clearArea(){
	$("#jjqBtn").attr("disabled", false);
	 	if (overlays.length >0 ) {
		  $.each(overlays,function(i,obj){
			  parent.window.map.removeOverlay(obj);
		  });
	}
	areaname = undefined;
	countArea = undefined;
...
/*触发点击第一个*/
function clickFirst() {
    $("#safeinfomenu ul li:first").click()/*.attr('class', 'active')*/;
}
        ...
        success: function (map) {
            ...
            map.forEach(function (item, index) {
                if (index > 1) {
                    var name = item.name;
                    var url = item.url;
                    Astr += '<li onclick="treeClick(\'' + name + '\',\'' + url + '\',this)">' + name + '</li>';
                }
            });
            $('#safeinfomenu').append("<ul class='allKinds'>" + Astr + "</ul>");
            $('.allKinds li:eq(0)').click();
...
/**点击权限树节点*/
function treeClick(name, url, item) {
//	var name = $(this).data('name');
//	var url = $(this).data('url');
    $('.ztree .allKinds li').removeClass('active');
    $(item).addClass('active');
    var Aarr = url.split('/map');
    var url = Aarr[0] + '/emsmap' + Aarr[1];
//	var tid = treeNode.tId;
//	$("li #"+tid+" a").attr("target","contentIframe");
//	$("li #"+tid+" a").attr("href","javascript:void(0)");
//	
...
//	}
//	
    $('.divContent').css('top', '5px');
    $('#jjqBtn').hide();
    $('#dlfsBtn').hide();
    if (name == "道路封锁") {
    	if (!dlfsAndjjqBtn) {
    		$('#dlfsBtn').show();
    		$('.divContent').css('top', '35px');
		}
    }
    if (name == "警戒区") {
    	if (!dlfsAndjjqBtn) {
