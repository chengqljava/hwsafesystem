...
var CameraHK={
    ...
    init:function(successCallback){//500宽  400高
    	var oheight = $(window).height()*0.6 - 38;
    	var owidth = $(window).width() - 205;
        if(CameraHK.isInstall()){
            WebVideoCtrl.I_InitPlugin(owidth, oheight, {
    ...
    login:function(ip,port,username,password,successCallback){
        ...
        var iRet = WebVideoCtrl.I_Login(ip, 1, port, username, password, {
            ...
            error: function () {
                if(flag){//不知道为什么 登录失败 error会执行2次 这样是为了只执行一次提示信息
                    flag = false;
                    console.log("登录失败,请检查用户名密码!");
                    var errorcode = WebVideoCtrl.I_GetLastError();
                    CameraHK.showOPInfo(ip+".."+username+".."+password+".."+port + " 登录失败！errorcode=="+errorcode);
                }
            }
        });
        if (-1 == iRet) {
