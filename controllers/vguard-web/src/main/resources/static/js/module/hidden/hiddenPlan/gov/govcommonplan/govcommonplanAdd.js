...
//	时间轴上的点击事件
  ...
  //三层页面保存按钮
    ...
    initPage(checkplanid);
});

function initPage(checkplanid){
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data) {
            	...
            	data.entids = entId;
            	var selectEntTb = _.template($("#selectEntTb").html());
				 ...
				 * 选择企业列表下一步操作
				 */
				$('#qixiangTypeAnaBtn').on('click', function () {
...
function save(){
	...
	$.each(rowIdArray,function(i,item){
		if($.inArray(item.CHECKTASKITEMID,checkTaskArr) != '-1'){			
			checkTaskArr.splice($.inArray(item.CHECKTASKITEMID,checkTaskArr),1);
		}
	});
	...
	if(curSelRowArr != null){			
		$.each(curSelRowArr,function(i,item){
			var checktaskitemid = $("#taskGrid-table").jqGrid("getRowData", item).CHECKTASKITEMID;
	...
	var entlistData = {name: "entArr", value: $("#optgroup").val()};
	formData.push(entlistData); 
	formData.push({name: 'creatergflag', value:creatergflag});
    //保存操作
	$.ajax({
		type : 'post',
...
function planCheckEnt(){
	$.ajax({
        type: 'POST',
        url: BASE_URL + '/hidden/hidcheckplan/planEntZtree'/*'/system/sysorg/orgtree'*/,
        cache: false,
        dataType: 'json',
        global: false,
        async: false,
