...
//		}
//	},,
	//{name:'MOBILECODE',index:'MOBILECODE',width:'10%',align:'center'},'移动终端'			exectype:$("#exectype").val(),
	
	var colname = ['人员id','姓名','区域','执法部门','办公室电话','手机号码','执法证号']; 
	var colmodel = [
		{name:'USERID',index:'USERID', width:'5%',hidden: true},
		{name:'NICKNAME',index:'NICKNAME',width:'10%',align:'left',
			formatter:function(cellvalue, options, obj) { 
				return '<a href="javascript:void(0);" onclick="display(\''+obj.USERID+'\')">'+obj.NICKNAME+'</a>';
			}
		},
		{name:'DISTRICTNAME',index:'DISTRICTNAME',width:'20%',align:'left'},
		{name:'DEPARTNAME',index:'DEPTNAME',width:'20%',align:'center'},
...
$("#addBtn").on("click", function () {
	parent.openWin(BASE_URL+"/law/lawuser/add",'添加','70%','80%');
});
...
$("#editBtn").on("click", function () {
	...
	var lawuserid = rowdata.USERID;
	
	parent.openWin(BASE_URL+'/law/lawuser/edit/'+lawuserid,'编辑','70%','80%');
});
...
$("#delBtn").on("click", function () {
	...
	for(var i=0;i<ids.length;i++){
		...
		lawuserids[i]= rowdatas.USERID;
	}
	var parmJson = lawuserids.toString();
...
function del(param){
	//查询是否有关联引用	
	$.ajax({ 
	  		url: BASE_URL+"/law/lawuser/loadLinkById",
	  		type:'post',
	  		dataType:'json',
	  		data:param,
	  		success: function(json){
	  			if(json.success==false){
	  				...
	  				parent.confirm("确认删除吗?", function() { 
	  					 $.ajax({ 
	  						 	url: BASE_URL+"/law/lawuser/delete",
	  					  		type:'post',
	  					  		dataType:'json',
	  					  		data:param,
	  					  		success: function(json){
