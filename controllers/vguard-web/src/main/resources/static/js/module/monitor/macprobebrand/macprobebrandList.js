$(document).ready(function() {
    ...
    $("#grid-table").jqGrid({
    	height: tableHeight,
    	url : BASE_URL + "//monitor/macprobebrand/list",
		datatype: "json",
		cache : false,
		mtype : 'post',
		colNames:colname,
...
$("#addBtn").on("click", function () {
	parent.openWin(BASE_URL+"//monitor/macprobebrand/add",'添加','50%','45%');
});
...
$("#editBtn").on("click", function () {
	...
	parent.openWin(BASE_URL+'//monitor/macprobebrand/edit/'+brandid,'编辑','50%','45%');
});

/*详细查询*/
function display(brandid){
	parent.openWin(BASE_URL+"//monitor/macprobebrand/display/"+brandid,'详细','50%','45%');
}
...
$("#delBtn").on("click", function () {
	...
	for(var i=0;i<ids.length;i++){
		...
		brandids[i]= rowdatas.brandid;
	}
	var parmJson = brandids.toString();
...
function del(param){
	$.ajax({ 
  		url: BASE_URL+"/monitor/macprobebrandtype/isHaveBrandType",
  		type:'post',
  		dataType:'json',
  		data:param,
  		success: function(json){
  			if(json.success==true){
	  			...
	  			parent.confirm('确认删除吗?',function(){
	  				$.ajax({ 
	  			  		url: BASE_URL+"/monitor/macprobebrand/delete",
	  			  		type:'post',
	  			  		dataType:'json',
	  			  		data:param,
	  			  		success: function(json){
