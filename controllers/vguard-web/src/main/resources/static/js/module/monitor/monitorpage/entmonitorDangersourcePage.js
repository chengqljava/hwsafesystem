...
function initVideoList(callBack){
	...
	$.ajax({
		type: 'post',
		url: BASE_URL + '/monitor/macmonitormap/loadMacVideoByEntid',
		cache: false,
		dataType: 'json',
		data: {
//			"entname": $("#entname").val(),
...
//							"<td title="+videoInfo.ENTNAME+">"+videoInfo.ENTNAME+"</td>" +
							"<td title="+videoInfo.DEVICENAME+">"+videoInfo.DEVICENAME+"</td>" +
			'<td><a onclick="showVideo(\'' + videoInfo.VIDEOID + '\',\'' + videoInfo.DEVICENAME +'\',\'' + videoInfo.HIKPLATNUM +'\',\'' + videoInfo.STRDOMAINCODE +'\',\'' + videoInfo.STRDEVICECODE +'\',\'' + videoInfo.STRCHANNELCODE +'\',\'' + videoInfo.STRSTREAMCODE +'\',\'' + videoInfo.HYPLAYPORT +'\',\'' + videoInfo.IPADDR + '\');"' +
				...
				//初始化显示第一个摄像头
//				showVideo(data[1].VIDEOID,data[1].VIDEONAME);
			}else{
				$("#titContent").html("");
...
//        });*/
//    	$("#titContent").html("");
//		$("#titContent").append('<font style="color: white;font-size: 20px">插件没有安装,请<a href="'+BASE_URL+'/monitor/macmonitormap/downloadWebVideo">下载</a>安装</font>');
...
//	    	window.OCXobj.StartTask_Preview_FreeWnd(previewXml);
	    } catch (e) {
	    	$("#titContent").html("");
			$("#titContent").append("<font style='color: white;font-size: 20px'>插件没有安装,请<a href='" + BASE_URL + "/monitor/macmonitormap/downloadWebVideo?type=8200'>下载</a>安装</font>");
			$("#titContent").addClass("noPlug");
