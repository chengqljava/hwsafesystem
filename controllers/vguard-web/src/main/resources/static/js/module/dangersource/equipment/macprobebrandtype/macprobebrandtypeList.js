...
//	SelectOption.loadProbeDeviceType("type");
	initSeachInput();
	var colname = ['主键id','所属品牌','上级菜单','型号名称','型号编码','主机/探头','备注']; 
	var colmodel = [
					...
					{name:'TYPE',index:'TYPE',align:'left', 
						formatter:function(cellvalue, options, obj) { 
							return SelectOption.getProbeDeviceType(obj.TYPE);
						}
					},
    ...
    $("#grid-table").jqGrid({
    	height: tableHeight,
    	url : BASE_URL + "/monitor/macprobebrandtype/list",
		datatype: "json",
		cache : false,
		mtype : 'post',
		colNames:colname,
...
$("#addBtn").on("click", function () {
	...
	parent.openWin(BASE_URL+"/monitor/macprobebrandtype/add?parentid="+$("#parentid").val()+"&brandid="+$("#brandid").val(),'添加','45%','50%');
});
...
$("#editBtn").on("click", function () {
	...
	parent.openWin(BASE_URL+'/monitor/macprobebrandtype/edit/'+brandtypeid+"?parentid="+$("#parentid").val()+"&brandid="+$("#brandid").val(),'编辑','45%','50%');
});

/*详细查询*/
function display(brandtypeid){
	parent.openWin(BASE_URL+"/monitor/macprobebrandtype/display/"+brandtypeid,'详细','45%','50%');
}
...
function del(param){
	...
	parent.confirm('确认删除吗?',function(){
		$.ajax({ 
	  		url: BASE_URL+"/monitor/macprobebrandtype/delete",
	  		type:'post',
	  		dataType:'json',
	  		data:param,
