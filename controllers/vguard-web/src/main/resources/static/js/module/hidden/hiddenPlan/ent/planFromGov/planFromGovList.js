	...
	$("#addBtn").off("click").on("click", function (){
		if (tabFlag == "cyc") {
			parent.openWin(BASE_URL
					+ "views/module/hidden/hiddenPlan/gov/govcycleplan/addCyclePlan.html?checkplanid=-1&creatergflag=2&pageIndex="+tabFlag,
					'新增周期检查计划', '53%', '73%');
		}else{
			parent.openWin(BASE_URL
		            + "views/module/hidden/hiddenPlan/gov/govcommonplan/govcommonplanAdd.html?checkplanid=-1&creatergflag=2&pageIndex="+tabFlag,
		            '新增安全检查计划', '50%', '75%');
		}
    });
	/**
        ...
        //返回当前grid中复选框所选择的数据的id
		 if (tabFlag == "com") {
			 var curSelRowArr = $("#conmmonplan-table").jqGrid("getGridParam", "selarrrow");
		    	...
		    	var planstate = $("#conmmonplan-table").jqGrid("getRowData", curSelRowArr[0]).PLANSTATE;
		    	if(planstate == "1"){
		        ...
		        parent.openWin(BASE_URL + "views/module/hidden/hiddenPlan/gov/govcommonplan/govcommonplanAdd.html?checkplanid=" + checkplanid+"&pageIndex="+tabFlag,
		            "编辑安全检查计划", "65%", "80%");
		 }else{
            ...
            // 弹出提示信息
			 ...
			 parent.openWin(BASE_URL + "views/module/hidden/hiddenPlan/gov/govcycleplan/addCyclePlan.html?checkplanid=" + checkplanid+"&pageIndex="+tabFlag,
        		"编辑周期检查计划", '55%', '75%');
		 }
    });
	 ...
	 $("#delBtn").off("click").on("click", function () {
		 ...
		 if (tabFlag == "com") {
			 ...
			 for (var i = 0; i < curSelRecordArr.length; i++) {
				 ...
				 var planstate = $("#conmmonplan-table").jqGrid("getRowData", curSelRecordArr[i]).PLANSTATE;
				 if(planstate == "1"){
					 parent.toast("计划已开始执行，不可删除！");
					 return;
				 } else if(planstate == "2"){
					 parent.toast("计划已执行完成，不可删除！");
					 return;
				 }
		     ...
		     if (0 == curSelRowArr.length) {
		       ...
		       for (var i = 0; i < curSelRowArr.length; i++) {
		    	   ...
		    	   var runningcount = $("#cycleplcn-table").jqGrid("getRowData", curSelRowArr[i]).RUNNINGCOUNT;
...
function downPlan(param) {
	parent.confirm("确认下发?下发后计划不可删除!", function () {
		$.ajax({
			type : "POST",
			async : false,
			dataType : "JSON",
...
//			},
	...
	$("#cycleplcn-table").jqGrid({
		height : tableHeight,
		url : BASE_URL + "hidden/hidcheckplan/loadGovForEntPlan",
		datatype : "json",
		cache : false,
		mtype : "POST",
		colNames : colname,
...
function displayHistory(checkplanid, checkname) {
	 parent.openWin(BASE_URL + "views/module/hidden/hiddenPlan/ent/entcycleplan/checkRecordList.html?checkplanid=" + checkplanid,
	    		checkname, "60%", "70%");
}
