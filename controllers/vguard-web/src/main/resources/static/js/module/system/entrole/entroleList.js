$(document).ready(function() {
	...
	//loadOrgTree("orgtree");
	    ...
	    $("#grid-table").jqGrid({
	    	height: tableHeight,
	    	url : BASE_URL + "/system/entrole/list",
			datatype: "json",
			cache : false,
			mtype : 'POST',
			colNames:colname,
...
$("#addBtn").bind("click",function(){
	parent.openWin(BASE_URL+"/system/entrole/add",'添加','60%','40%');
});
...
$("#editBtn").bind("click",function(){
	...
	parent.openWin(BASE_URL+"/system/entrole/edit/"+roleid,'编辑','60%','40%');
});
...
/*
	...
	*/
	...
	parent.openWin(BASE_URL+"/system/entrole/rolePriv/"+roleid,'权限分配','60%','50%');
});
...
function del(param){
	...
	$.ajax({ 
	  		url: BASE_URL+"/system/entrole/loadLinkById",
	  		type:'post',
	  		dataType:'json',
	  		data:param,
	  		success: function(json){
	  			if(json.success==false){
	  				...
	  				parent.confirm("确认删除吗?", function() { 
	  					 $.ajax({ 
	  						 	url: BASE_URL+"/system/entrole/delete",
	  					  		type:'post',
	  					  		dataType:'json',
	  					  		data:param,
	  					  		success: function(json){
...
function display(roleid){
	parent.openWin(BASE_URL+"/system/entrole/display/"+roleid,'详细','60%','45%');
}
