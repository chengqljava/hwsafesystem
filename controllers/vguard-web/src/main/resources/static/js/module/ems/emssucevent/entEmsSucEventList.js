$(document).ready(function() {
	...
	var colname = ['主键','事故名称','发生时间','发生地址','事故类型','事故等级','事故原因','事故概述','伤亡情况','状态',"操作"]; 
	var colmodel = [{name:'EVENTID',index:'EVENTID',hidden:true},
                    ...
                    {name:'STATE',index:'STATE',align: 'center', hidden:true,
                        formatter:function(cellvalue, options, obj) { 
            		...
            		{name:'OPERATION',index:"OPERATION",width:'80px',fixed:true,align:"left",formatter:function(cellvalue, options, obj){
            			return '<a href="javascript:void(0);" title="编辑" class="operation editInfoBtn" onclick="editInfo(\''+obj.EVENTID+'\')">编辑</a><br>'
            			     + '<a href="javascript:void(0);" title="删除" class="operation delInfoBtn" onclick="delInfo(\''+obj.EVENTID+'\')">删除</a>'
            		}}
//					{name: '', width: '200px', align: 'center', 
//            			formatter:function(cellvalue, options, obj) {
//                            return '<a href="javascript:void(0);" onclick="hanEmRes(\''+obj.EVENTID+'\')">应急救援</a>';
//                        }
//					}
			       ];
...
function editInfo(eventid) {
	parent.openWin(BASE_URL+'/ems/emssucevent/edit/'+eventid,'编辑','65%','75%');
}
// 删除
function delInfo(eventid) {
	var param = {"ids":eventid};
	del(param);
}
/*加载*/
function reloadGrid(){
 ...
 * @param eventid
 */
function hanEmRes(eventid) {
	var lastPageUrl = location.href;
	location.href = BASE_URL + "/views/module/ems/map/emResIndex.html?" +
					"lastPageUrl=" + lastPageUrl + "&eventid=" + eventid;
}
