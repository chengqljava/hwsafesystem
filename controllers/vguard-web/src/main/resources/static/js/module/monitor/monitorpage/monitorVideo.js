	...
	$.ajax({
		...
		success : function(data) {
			if (data) {
				console.log(data);
				if(data.length>0){					
					var macVideo = data.shift();
					showVideo(macVideo.VIDEOID, macVideo.VIDEONAME, macVideo.HIKPLATNUM,
							macVideo.STRDOMAINCODE, macVideo.STRDEVICECODE, macVideo.STRCHANNELCODE, 
							macVideo.STRSTREAMCODE, macVideo.HYPLAYPORT, macVideo.IPADDR);
				}
			}
		},
		error : function() {
