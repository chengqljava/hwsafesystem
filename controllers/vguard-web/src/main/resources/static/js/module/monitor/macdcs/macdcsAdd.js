 ...
 */
$(function () {
    var dcsid = getQueryString("dcsid");
    //获取服务器信息
    $.ajax({
        type: "post",
        url: BASE_URL + "monitor/macdcs/load",
        dataType: "json",
        data: {
            dcsid: dcsid
        },
        success: function (data) {
            if (data) {
                ...
                SelectOption.loadMacDcsMaster("probehostid");
                if(isEnt){
                    $(".ent").hide();
                    $("#dept").attr("colSpan",3);
                }else {
...
function save() {
    ...
    var data2 = $("#macdcsForm").serializeArray();
    $.each(data2,function (index, item) {
       data.push({name:item.name,value:item.value});
    });
    $.ajax({
        type : "post",
        url : BASE_URL + "monitor/macdcs/save",
        data : data,
        success : function(data) {
