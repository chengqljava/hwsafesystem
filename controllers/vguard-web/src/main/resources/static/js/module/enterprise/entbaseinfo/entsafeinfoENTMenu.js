$(document).ready(function() {
	...
	var status = $("#status").val();
	var businessinfoid = $("#businessinfoid").val(); //工商信息id
...
function treeClick(event,treeId, treeNode, clickFlag){
	...
	if(treeNode.id == -1||treeNode.name=="安全生产人员信息"||treeNode.name=="安全生产信息"||treeNode.name=="标准化制度及资料"||treeNode.name=="特种设备"){
		return false;
	}
	...
	if(treeNode.tablename != "ent_baseinfo"){
    ...
    if(nodeurl.endWith("html")){
    	if ($("#epiStatus").val() == '3') {
    		url = BASE_URL+nodeurl +"?businessinfoid="+businessinfoid+"&isDisplay=display";
		}else{
			url = BASE_URL+nodeurl +"?businessinfoid="+businessinfoid;
		}
    }
...
$("#report").on("click", function () {
	var businessinfoid = $('#businessinfoid').val(); //企业工商信息id
	...
	var ent_safeperson = $('#ent_safeperson').val(); //安全生产责任人(必填)
	var ent_safemanager = $('#ent_safemanager').val(); // 安全生产管理人员(必填)
	if(ent_baseinfo =="false" || ent_safeorg == "false" || ent_dangerchara == "false" 
		|| ent_safeperson == "false" || ent_safemanager == "false"){
		parent.parent.toast("请填写完必填项");
		return;
	}
	
	//弹出提示框
	parent.parent.confirm('确认上报吗?',function(){
		$.ajax({ 
	  		...
	  		success: function(json){
	  			if(json.success==true){
	  				...
	  				$('#update').css("display","block");
	  				$('#report').css("display","none");
	  				$("#epiStatus").val(json.epiStatus);
	  				
	  				var str = document.getElementById('contentIframe').contentWindow.location+"";
	  				if(str.indexOf("html") == -1) {
	  					document.getElementById('contentIframe').contentWindow.location.reload(true);
					}else{
						var location;
						if (json.epiStatus == '3') {
							location = str +"&isDisplay=display";
						}else{
							location = str;
						}
						document.getElementById('contentIframe').contentWindow.location = location;
					}
...
$("#update").on("click", function () {
	...
	//弹出提示框
	parent.parent.confirm('确认更新吗?',function(){
		$.ajax({ 
	  		url: BASE_URL+"/enterprise/entbaseinfo/update",
	  		type:'post',
	  		dataType:'json',
	  		data:{"businessinfoid":businessinfoid,"baseinfoflag":ent_baseinfo},
	  		success: function(json){
	  			if(json.success==true){
	  				parent.parent.toast(json.msg);
	  				$("#epiStatus").val(json.epiStatus);
	  				...
	  				if(str.indexOf("html") == -1) {
						...
						if (json.epiStatus == '3') {
							...
							location = str.substring(0,str.lastIndexOf("&"));
						}
						document.getElementById('contentIframe').contentWindow.location = location;
					}
