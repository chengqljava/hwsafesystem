...
function initWorkShopMonPts() {
    $('#viewArea').on('click', '.realWkSpMonPt', (function (e) {
        ...
        if (mactype == "1") {
            ...
            showVideoDiv(id, $(this).attr("data-hikType"), $(this).attr("data-pName"));
        }
    }));
}
 ...
 * @param entid
 */
function showVideoDiv(videoid, videoType, videoName) {
...
//    	setTimeout(function() {
//    		if (videoType) {
//    			playVideoByType(videoType, videoid);
//    		}
//    	}, 500);
    	
    	playHyVideo(videoName);
...
/**
 * 测试-根据怀业平台视频点位名称播放相应视频
 * @param videoName
 */
function playHyVideo(videoName) {
	try {
 			$("#hyPlayer").show();
			...
			setTimeout(function() {
				//停止上次的视频播放
		 		...
		 		if (window.hyVideoList) {
		 			var curhyVideo = _.where(window.hyVideoList, {"VIDEONAME": $.trim(videoName)});
 ...
 * @param videoid
 */
function playVideoByType(videoType, videoid) {
	if ("8200" == videoType) {
		 //82平台视频加载
		 $("#spvViewOCX").hide();
		 $("#PlayViewOCX").show();
