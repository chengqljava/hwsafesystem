$(document).ready(function() {
    ...
    $("#grid-table").jqGrid({
    	height: 450,
    	url : BASE_URL+"/hiddendanger/hdigovregistra/gov/check",
		datatype: "json",
		cache : false,
		mtype : 'POST',
		colNames:colname,
...
function checkyes(standardid,restate){
	if(verify(restate))
		parent.openWin(BASE_URL + '/hiddendanger/hdigovregistra/govadd/'+$('#inspectionid').val()+'/'+$('#itemid').val()+'/'+standardid+'/'+$('#rummager').val()+'/'+$('#checktime').val(), '隐患登记', '50%', '65%');
}
...
function checkno(standardid,restate){
	if(verify(restate)){
		...
		$.ajax({ 
	  		url: BASE_URL + "/hiddendanger/hdigovregistra/govnosave",
	  		type:'post',
	  		dataType:'json',
	  		data:param,
...
function del(registraid){
	...
	// 查询是否有隐患整改信息
	$.ajax({
		url : BASE_URL + "/hiddendanger/hdigovabarbeitung/loadByGovRegistraid",
		type : 'post',
		dataType : 'json',
		data : param,
		success : function(json) {
			if (json.success == false) {
				...
				//弹出提示框
  				parent.confirm('确认删除吗?',function(){
  					$.ajax({ 
  				  		url: BASE_URL + "/hiddendanger/hdigovregistra/delete",
  				  		type:'post',
  				  		dataType:'json',
  				  		data:param,
  				  		success: function(json){
 ...
 */
function config(){
	parent.openWin(BASE_URL + '/hiddendanger/hdigovregistra/config/'+$('#businessinfoid').val(), '自查指引配置表', '50%', '60%');
}
...
function otherpage(num){
	...
	// 其它
	if(num == '4'){
		parent.openWin(BASE_URL + '/hiddendanger/hdigovregistra/otherpage/'+$('#inspectionid').val()+"/"+$('#businessinfoid').val(), '其它检查信息', '50%', '60%');
	}
}
...
function loadCheckTree(){
	...
	$.ajax({
		type :'post',
		url : BASE_URL+'/hiddendanger/hdicheckitem/checkitemgovtree',
		cache : false,
		dataType : 'json',
		data : {
			"businessinfoid" : $('#businessinfoid').val(),
