/**
 * 应急救援GIS首页-智能方案-组建指挥部弹窗 Created by Administrator on 2017/10/24.
 */
$(function() {
	...
	if ("0" != schemaId) {
		...
		$(window).resize(function() {
					$("#grid-table").jqGrid("setGridHeight", 140);
					$("#grid-table").jqGrid("setGridWidth", $(window).width() - 30 );
				});
		...
		$("#grid-table").jqGrid({
			...
			ondblClickRow : function(rowid, iRow, iCol, e) {
				...
				event.resourcename = event.deptname;
											...
											.ajax({
												...
												success : function(data) {
													if (data.success == true) {
														...
														if ("1" == isCallBack) {
															window.top.GEventObject
																	.fireEvent(
																			"EMS_AiPLAN_TASKNODECLK_EVENT",
																			"2");
														}
													}
...
function initTaskGird(schemaId, curNode) {
	var colname = [ '主键', '方案ID', '方案节点', '接收人id', '接收人', '下发时间', '完成时间',
			'任务状态', '任务跟踪', '任务内容' ];
	var colmodel = [
			{
				name : 'TASKID',
			...
			},
			{
				...
				formatter : function(cellvalue, options, obj) {
					if (cellvalue) {
						return getFormatDateByLong(cellvalue,
								"yyyy-MM-dd hh:mm:ss");
					} else {
						return "-";
					}
			...
			},
			{
				...
				align : 'center',
				formatter : function(cellvalue, options, obj) {
					return '<a href="javascript:void(0);" onclick="traceTask(\''
							+ obj.TASKID
							+ '\',\''
							+ obj.TASKRECEIVER
							+ '\')">跟踪</a>';
	...
	$("#tskGrid-table").jqGrid(
			{
				...
				loadComplete : function() {
					if ($(window).width() < 400) {
						...
						$("#tskGrid-table").closest(".ui-jqgrid-bdiv").css({
							"overflow-x" : "scroll",
							"height" : "350"
						});
						$(".ui-jqgrid-hdiv").css({
						...
						$("#tskGrid-table").jqGrid('setGridWidth',
								$(window).width() * 0.99);
					}
				}
			});
