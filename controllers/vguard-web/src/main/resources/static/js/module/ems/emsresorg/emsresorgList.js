$(document).ready(function() {
	...
	var colname = ['机构id','机构编号','机构名称','行政区域','机构职责','内设部门','机构成员','地图定位','所属单位','填报状态',"操作"]; 
	var colmodel = [
		...
		{name:'LONGITUDE',index:'LONGITUDE',width:'10%',align:'center',hidden: true,
			formatter:function(cellvalue, options, obj) { 
		...
		{name:'UNITNAME',index:'UNITNAME',width:'20%',align:'left',hidden: true},
		{name:'FILLSTATE',index:'FILLSTATE',width:'10%',align:'center',hidden: true,
			formatter:function(cellvalue, options, obj) { 
				if(obj.FILLSTATE=='1'){
					return '填报中';
				}else{
		...
		{name:'OPERATION',index:"OPERATION",width:'80px',fixed:true,align:"left",formatter:function(cellvalue, options, obj){
			return '<a href="javascript:void(0);" title="编辑" class="operation editInfoBtn" onclick="editInfo(\''+obj.ORGID+'\')">编辑</a><br>'
			     + '<a href="javascript:void(0);" title="删除" class="operation delInfoBtn" onclick="delInfo(\''+obj.ORGID+'\')">删除</a>'
		}}
	];
    ...
    $("#grid-table").jqGrid({
    	height: tableHeight,
    	url : BASE_URL + "/ems/emsresorg/list",
		datatype: "json",
		cache : false,
		mtype : 'post',
		colNames:colname,
